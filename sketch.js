...
function generateQuestions() {
    ...
    // Mooie coëfficiënt weergave: geen 1x, wel -1x → -x
    function coefStr(a, variable) {
        if (a === 0) return "";
    ...
    // Term weergave: coëfficiënt × variabele^macht
    function termStr(a, macht) {
        if (a === 0) return "";
        if (macht === 0) return "" + a;
        if (macht === 1) return coefStr(a, "x");
        return coefStr(a, "x^" + macht);
    }

    // Som van termen naar string (+ en - tekens goed zetten)
    function polyStr(termen) {
        // termen = [{a, n}, {a, n}, ...]
        let parts = termen
            .filter(t => t.a !== 0)
            .map(t => termStr(t.a, t.n));
    ...
    // Afgeleide van polynoom [{a,n}] → [{a*n, n-1}]
    function differentieer(termen) {
        return termen
            .filter(t => t.n > 0)
            .map(t => ({ a: t.a * t.n, n: t.n - 1 }))
            .filter(t => t.a !== 0);
    }

    // Willekeurig geheel getal tussen min en max (niet 0)
    function randNonZero(min, max) {
        let v;
        do { v = floor(random(min, max + 1)); } while (v === 0);
        return v;
    }
    ...
    // ======================================
    // VRAAGTYPE 1: Polynoom  ax^n + bx^m + c
    // ======================================
    function maakPolynoom() {
        let hoogsteMacht = randInt(2, 5);           // max macht 5
        let a = randNonZero(-5, 5);                 // kleine coëf
        let termen = [{ a: a, n: hoogsteMacht }];

        let tweedeMacht = randInt(1, hoogsteMacht - 1);
        let b = randNonZero(-5, 5);
        termen.push({ a: b, n: tweedeMacht });
        ...
        let vraag = "f(x) =\n " + polyStr(termen);
        let afgeleide = differentieer(termen);
        let antwoord = afgeleide.length === 0 ? "f'(x) =\n 0" : "f'(x) =\n " + polyStr(afgeleide);
        return { text: vraag, answer: antwoord };
    }

    function maakProduct() {
        // Beperk tot kleine coëfficiënten zodat uitgewerkte termen < 50 blijven
        let a = randNonZero(-4, 4);
        let b = randNonZero(-6, 6);
        let c = randNonZero(-4, 4);
        let d = randNonZero(-6, 6);

        // Vermijd te grote coëfficiënten in het antwoord
        // Afgeleide wordt: 2ac·x + (ad+bc)
        // Controleer of die binnen bereik blijven
        if (Math.abs(2 * a * c) > 20 || Math.abs(a * d + b * c) > 40) {
            return maakProduct(); // probeer opnieuw
        }

        function factorStr(p, q) {
            let pStr = p === 1 ? "x" : p === -1 ? "-x" : p + "x";
            if (q > 0) return "(" + pStr + " + " + q + ")";
            if (q < 0) return "(" + pStr + " - " + Math.abs(q) + ")";
            return "(" + pStr + ")";
        }

        let vraag = "f(x) =\n " + factorStr(a, b) + factorStr(c, d);
        ...
        let afgeleide = differentieer(uitgewerkt);
        let antwoord = afgeleide.length === 0 ? "f'(x) =\n 0" : "f'(x) =\n " + polyStr(afgeleide);
        return { text: vraag, answer: antwoord };
    }

    function maakHaakjesMacht() {
        let n = randInt(2, 3);
        // Kleine waarden zodat uitgewerkte coëfficiënten beheersbaar blijven
        let a = randNonZero(-3, 3);
        let b = randNonZero(-3, 3);
        let c = randNonZero(-4, 4);

        // Controleer of de afgeleide-coëfficiënten niet te groot worden
        let maxCoef;
        if (n === 2) {
            maxCoef = Math.max(
                Math.abs(2 * a * b * b),
        ...
        if (maxCoef > 50) return maakHaakjesMacht(); // probeer opnieuw

        function haakjesStr(p, q) {
            let pStr = p === 1 ? "x" : p === -1 ? "-x" : p + "x";
            if (q > 0) return "(" + pStr + " + " + q + ")";
            if (q < 0) return "(" + pStr + " - " + Math.abs(q) + ")";
            return "(" + pStr + ")";
    ...
    // ======================================
    // SELECTEER 10 VRAGEN (mix van 3 typen)
    // ======================================
    let questions = [];

    // Garanteer minimaal 3 van elk type
    for (let i = 0; i < 4; i++) questions.push(maakPolynoom());
    for (let i = 0; i < 3; i++) questions.push(maakProduct());
    for (let i = 0; i < 3; i++) questions.push(maakHaakjesMacht());

    // Schud de volgorde door elkaar
    questions = shuffle(questions);

    // Antwoorden losgekoppeld schudden voor de oranje blokjes
    let answers = shuffle(questions.map(q => q.answer));
    ...
    // ===== MAAK ANTWOORD BLOKJES (rijen 3 en 4) =====
    let answerIndex = 0;
...
function drawBlock(block) {
  ...
  // ====== SUPERSCRIPT TEKENFUNCTIE ======
  function tekenWiskundeText(txt, cx, cy) {
    // Splits tekst in regels (op \n)
    let regels = txt.split('\n');
    let normalSize = 15;
    let supSize = 8;
    let regelHoogte = 24;
    let startY = cy - ((regels.length - 1) * regelHoogte) / 2;

    for (let r = 0; r < regels.length; r++) {
      let regel = regels[r];
      let regelY = startY + r * regelHoogte;

      // Splits regel in tokens: normaal tekst en superscripts (^getal)
      let tokens = [];
      let remaining = regel;
      let supRegex = /\^(\d+)/;

      while (remaining.length > 0) {
        let match = supRegex.exec(remaining);
        if (match) {
          if (match.index > 0) {
            tokens.push({ type: 'normal', text: remaining.substring(0, match.index) });
          }
          tokens.push({ type: 'sup', text: match[1] });
          remaining = remaining.substring(match.index + match[0].length);
        } else {
          tokens.push({ type: 'normal', text: remaining });
          remaining = '';
        }
      }

      // Meet totale breedte van deze regel
      let totalW = 0;
      textSize(normalSize);
      for (let tok of tokens) {
        if (tok.type === 'normal') {
          totalW += textWidth(tok.text);
        } else {
          textSize(supSize);
          totalW += textWidth(tok.text);
          textSize(normalSize);
      ...
      // Teken tokens gecentreerd op cx
      let drawX = cx - totalW / 2;
      for (let tok of tokens) {
        if (tok.type === 'normal') {
          textSize(normalSize);
          textAlign(LEFT, CENTER);
          text(tok.text, drawX, regelY);
          drawX += textWidth(tok.text);
        } else {
          textSize(supSize);
          textAlign(LEFT, CENTER);
          text(tok.text, drawX, regelY - 5);
          drawX += textWidth(tok.text);
          textSize(normalSize);
        }
      }
  ...
  // ====== TEKST POSITIE ======
  if (block.isQuestion && block.hoverProgress > 0) {
    tekenWiskundeText(block.text, CELL_SIZE / 2, CELL_SIZE / 2);
  } else {
    tekenWiskundeText(block.text, block.x + CELL_SIZE / 2, block.y + CELL_SIZE / 2);
  }
